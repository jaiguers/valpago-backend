...
type Config struct {
	...
	RedisNotificationsStream string
	RedisGroup               string
...
func Load() error {
	...
	C.RedisURL = getenv("REDIS_URL", "redis://127.0.0.1:6379") // Para Redis Cloud usar: rediss://username:password@host:port
	C.RedisStreamNS = getenv("REDIS_STREAM_NAMESPACE", "valpago:transactions")
	C.RedisNotificationsStream = getenv("REDIS_NOTIFICATIONS_STREAM", "valpago:notifications")
